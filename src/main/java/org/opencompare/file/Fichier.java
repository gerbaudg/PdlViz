...
public class Fichier {
	...
	private FonctionnementInterface fonctionnement;

	public Fichier(FonctionnementInterface fonctionnement2) {
		fonctionnement = fonctionnement2;
	}
	...
	public void createHtml(int nbFeatures) throws IOException {
		...
		body = "<body>" + "<script src=\"https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.4.0/Chart.min.js\"></script>"
				+ "<script src=\"" + jsPathRelative + "\" ></script>" + bouton + "<button onclick=\"" + execFunction
				+ "\";><img src=\"img/bars-chart.png\" width=\"32px\" height=\"32px\" class=\"coinphoto\"></button>"
				+ "<button onclick=\"" + execFunction
				+ "\";><img src=\"img/pie-chart.png\" width=\"32px\" height=\"32px\" class=\"coinphoto\"></button>"
				+ "<div style=\"width : 400px\">" + "<canvas id=\"" + idChart
	...
	public void createJS() throws IOException {
		...
		chartFunction = "function " + execFunction + " {var jsonData = $.ajax({url: '" + jsonPath
				+ "',dataType: 'json',}).done(function (results) {var feature = \"nb de porte\";\r\n"
				+ "		var dataset = [];\r\n" + "		var labels = [];\r\n" + "		var data = [];\r\n"
		...
		File file = new File(jsPath + "newChart.js");
		// creates the file
		file.createNewFile();
	...
	public void createRadarJS() throws IOException {
		// POur chaque product
		Map<String, List<String[]>> mapRadar = fonctionnement.genererRadarForProduct();

		Set<String> keyset = mapRadar.keySet();
		Iterator<String> it = keyset.iterator();

		String global = "Chart.defaults.scale.ticks.beginAtZero = true;Chart.defaults.global.elements.line.borderWidth = 1;";
		String functions = "function getRandomColor() {var letters = '0123456789ABCDEF';var color = '#';for (var i = 0; i < 6; i++ ) {color += letters[Math.floor(Math.random() * 16)];}return color;}";
		String labels = "", data = "";

		// Pour chaque produit -> un js
		while (it.hasNext()) {
			String produit = it.next();
			List<String[]> donnee = mapRadar.get(produit);
			Iterator<String[]> it2 = donnee.iterator();
			if (it2.hasNext()) {
				String[] tab = it2.next();
				labels = "[\"" + tab[0] + "\"";
				data = "[" + tab[1];
			...
			String bigData = "{ " + "labels : " + labels + "datasets : [" + "{ label : \"" + produit + "\"," + "data :"
					+ data + "}]}";

			String body = "function " + execFunction + "{" + "	 var ctx = document.getElementById(\"" + idChart
					+ "\").getContext(\"2d\");\r\n" + "     var " + idChart + " =new Chart(ctx, {\r\n"
					+ "		type: 'radar',\r\n" + "		data: " + bigData + "\r\n" + "	});\r\n" + "	\r\n" + "}";

			File file = new File(jsPath + "radar" + produit + ".js");
			// creates the file
			file.createNewFile();
			...
			// Writes the content to the file
			writer.write(global + functions + body);
